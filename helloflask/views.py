...
from helloflask.models import User, Song, Album, Artist, SongArtist, SongRank, Myalbum, Mycom, SongInfo
...
def songlist(dt):
    ...
    # sr = SongRank.query.options(joinedload(SongRank.song))
    sr = sr.options(joinedload(SongRank.song, Song.album))
    sr = sr.options(joinedload(SongRank.song, Song.songartists))
    sr = sr.filter(text('atype=1'))

    # sr = sr.filter(text('atype=0')).params(atype=0)  # 아래 위는 모두 같은 의미임
...
def myalbum():
    if not session.get('loginUser'):
        session['next'] = request.url  # session 으로 박아둔다.
        return redirect('/login')
        # return redirect( url_for('login', next=request.url) )  # 이것도 사용가능함

    loginUser = session.get('loginUser')
    loginId = loginUser.get('userid')
    songs = Myalbum.query.filter(text('userid=:userid')).params(userid=loginId).all()

    if request.is_xhr:  # ajax 로 호출하였으먄 json 으로 내려보내라.
...
@app.route('/')
def idx():
    lives = songlist('2022-10-28')
    todays = songlist('2022-9-28')
